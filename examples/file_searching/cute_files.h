    ...
    do
    {
        if (n >= max)
        {
            if (! CUTE_FILES_DEBUG_CHECKS)
                break;
            printf(
                "ERROR: String \"%s\" too long to copy on line %d in file %s (max length of %d).\n",
                original,
                line,
                file,
                max);
...
const char* cf_get_ext(cf_file_t* file)
{
    ...
    while (*name++)
        if (*name == '.')
            period = name;
    if (period)
        cf_safe_strcpy(file->ext, period, 0, CUTE_FILES_MAX_EXT);
    else
        file->ext[0] = 0;
    return file->ext;
}
...
int cf_match_ext(cf_file_t* file, const char* ext) { return ! strcmp(file->ext, ext); }
...
int cf_compare_file_times_by_path(const char* path_a, const char* path_b)
{
    ...
    if (GetFileAttributesExA(path_a, GetFileExInfoStandard, &data))
        time_a = data.ftLastWriteTime;
    if (GetFileAttributesExA(path_b, GetFileExInfoStandard, &data))
        time_b = data.ftLastWriteTime;
...
// Warning : untested code! (let me know if it breaks)
int cf_compare_file_times(cf_time_t* time_a, cf_time_t* time_b) { return (int)difftime(time_a->time, time_b->time); }

// Warning : untested code! (let me know if it breaks)
int cf_file_exists(const char* path) { return access(path, F_OK) != -1; }
